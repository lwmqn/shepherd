    ...
    if (!node) {
        ...
        node.dbRemove().done(function () {
            ...
            self._responseSender('deregister', clientId, { status: mutils.rspCodeNum('Deleted') }).done(function () {
                ...
                self.emit('ind:leaving', clientId);
            });
        }, function (err) {
            ERR(err);
            deferred.reject(err);
...
/*** Server Information                                                                        ***/
/*************************************************************************************************/
MShepherd.prototype.getInfo = function (callback) {
    var self = this;
    ...
    network.get_active_interface(function(err, obj) {
        callback(err, {
            manuf: 'LWMQN Project',
            sw: 'v1.0.0',
            state: self._enabled ? 1 : 0,   // 0: up, 1: down
            intf: obj.name,
            routerIp: obj.gateway_ip,
            ip: obj.ip_address,
            mac: obj.mac_address,
            type: obj.type,
            permitRemainingTime: self._permitJoinTime
        });
    });
};
...
// (ok) 'ready'  IND:NET_READY SYS_READY_IND
// 'error'
// (ok) 'message'

// (ok) 'ind:incoming'  IND:DEVICE_INCOMING, NWK_DEVICE_IND
//      >> .emit('ind:incoming', qnode)
// (ok) 'ind:leaving'   IND:DEVICE_LEAVING
//      >> .emit('ind:leaving', clientId)
// (ok) 'ind:updated'   IND:NET_CHANGED, DEV_ATTRIBUTE_CHANGE_IND, DEV_NET_CHANGE_IND, (DEV_STATE_CHANGE_IND)
//      >> .emit('ind:updated', qnode, data)
// (ok) 'ind:notified'  IND:REPORT, DEV_ATTRIBUTE_REPORTING_IND
//      >> .emit('ind:notified', qnode, msg)
// 'ind:changed'   IND:ATTR_CHANGED
// (ok) 'ind:status'    STATUS_CHANGED, (DEV_STATE_CHANGE_IND)

// IND:SLEEPY_DEVICE_CHECKIN, DEV_SLEEPY_DEVICE_CHECK_IN_IND
