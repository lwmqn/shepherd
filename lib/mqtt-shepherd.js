...
function MShepherd(name, settings) {
    ...
    // [TODO2] Cannot load all nodes after broker ready, since we may lost some sign in messages
...
MShepherd.prototype.start = function (callback) {
    ...
    if (!this.mBroker)
        this.mBroker = new mosca.Server(this.brokerSettings);
...
function _testShepherdPubSub(shepherd, callback) {
    ...
    testMsgListener = function (topic, message, packet)  {
        ...
        if (checkCount === totalCount) {
            ...
            SHP('7. Shepherd pub/sub test result: OK');
            deferred.resolve();
        }
    };
...
function _clientRegisterHandler(shepherd, msg) {
    ...
    if (!node && msg.clientId) {
        ...
        msg.objList = _returnObjListOfSo(msg.objList);
...
function _returnObjListOfSo(objList) {
    var oList = {};
