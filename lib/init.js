...
const debug = require('debug')('lwmqn-shepherd:init')

const mutils = require('./components/mutils')
const msghdlr = require('./components/msghandler')
const MqttNode = require('./components/qnode')
...
init.setupShepherd = function (shepherd, callback) {
  ...
  debug('lwmqn-shepherd booting...')

  const initProcedure = function () {
    init._setupAuthPolicy(shepherd).then(() => { // 1. set up authorization for priphs
      ...
      .done(() => {
        debug('lwmqn-shepherd is up and ready.')
        deferred.resolve()
      })
  }
...
init._setupAuthPolicy = function (shepherd) {
  ...
  broker.authorizePublish = function (client, topic, payload, cb) {
    const validTopic = mutils.slashPath(topic)
    ...
    if (client.id === shepherdId) { // shepherd can always publish
      ...
      if (shepherd.authPolicy && _.isFunction(shepherd.authPolicy.authorizePublish)) shepherd.authPolicy.authorizePublish(client, topic, payload, cb)
      else cb(null, true)
    }
  }
  ...
  broker.authorizeSubscribe = function (client, topic, cb) {
    ...
    if (client.id === shepherdId) { // shepherd can always subscribe
      ...
      if (shepherd.authPolicy && _.isFunction(shepherd.authPolicy.authorizeSubscribe)) shepherd.authPolicy.authorizeSubscribe(client, topic, cb)
      else cb(null, true)
    }
  }
...
init._testShepherdPubSub = function (shepherd) {
  ...
  const testMsgListener = function (topic, message, packet) {
...
init._attachShepherdMessageHandler = function (shepherd) {
  ...
  mc.on('message', (topic, message, packet) => {
    ...
    shepherd.decrypt(message, cId, (err, decrypted) => {
      if (err) debug(`Decryption fails. From client id: ${cId}`) // log 'decryption fails'
  ...
  if (mqdb) {
    mqdb.exportClientIds().then((cIds) => {
      _.forEach(cIds, (clientId) => {
        const resNode = shepherd._nodebox[clientId] = new MqttNode(shepherd, clientId)
        resNode._setStatus('offline')
        const doRestore = resNode.restore().then(() => {
