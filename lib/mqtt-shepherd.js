...
MShepherd.prototype.start = function (callback) {
    ...
    initProcedure = function () {
        ...
        _setupAuthPolicy(shp).then(function () {        // 1. set up authorization for priphs
            _.forEach(brokerEvtHdlrsToRemove, function (evt) {
                broker.removeAllListeners(evt);         // 2. remove all listeners attached on broker
            });
            return true;
        }).then(function () {
            return _attachBrokerEventListeners(shp);    // 3. re-attach listeners:
        }).then(function () {
            return _setShepherdAsClient(shp);           // 4. let shepherd in
        }).then(function () {
            return _testShepherdPubSub(shp);            // 5. run shepherd pub/sub testing
        }).delay(1000).then(function () {
            return _attachShepherdMessageHandlers(shp);
    ...
    _loadNodesFromDb(shp).then(function () {
        if (!shp.mBroker)
...
MShepherd.prototype._requestSender = function (cmdId, clientId, reqObj, callback) {
    ...
    reqObj.cmdId = _.isUndefined(mutils.cmdIdNum(cmdId)) ? cmdId : mutils.cmdIdNum(cmdId); // 255: unknown cmd
...
function _setupAuthPolicy(shp) {
    ...
    broker.authenticate = function (client, user, pass, cb) {
        ...
        // [DEBUG] if (dbg_mode) return 'authenticate';
    };
    ...
    broker.authorizePublish = function (client, topic, payload, cb) {
        ...
        // [DEBUG] if (dbg_mode) return 'authorizePublish';
    };
    ...
    broker.authorizeSubscribe = function (client, topic, cb) {
        ...
        // [DEBUG] if (dbg_mode) return 'authorizeSubscribe';
    };
    ...
    broker.authorizeForward = function (client, packet, cb) {
        ...
        // [DEBUG] if (dbg_mode) return 'authorizeForward';
    };
...
function _attachBrokerEventListeners(shp) {
    var deferred = Q.defer(),
        shpId = shp.clientId,
        broker = shp.mBroker;
...
/*************************************************************************************************/
function _setShepherdAsClient(shp) {
    var deferred = Q.defer(),
        shpId = shp.clientId,
    ...
    mc.on('close', function () {
        MC(shpId + ' is disconnected from the broker.');
    });
    ...
    mc.on('reconnect', function () {
        MC(shpId + ' is re-connecting to the broker.');
    });

    mc.on('connect', function (connack) {
        MC(shpId + ' is connected to the broker.');
...
function _testShepherdPubSub(shp) {
    var deferred = Q.defer(),
        mc = shp.mClient,
        shpId = shp.clientId,
...
function _attachShepherdMessageHandlers(shp) {
    var deferred = Q.defer(),
        mc = shp.mClient;
    ...
    mc.on('error', function (err) {
        MC('Error occured when ' + shp.clientId + ' was connecting to the broker.');
        shp.emit('error', err);
    });
    ...
    mc.on('message', function (topic, message, packet) {
        ...
        switch (intf) {
            ...
            case 'response':
                ...
                parsedMsg.cmdId = mutils.cmdIdKey(parsedMsg.cmdId);
...
function _clientResponseHandler(shepherd, msg) {
    ...
    var clientId = msg.clientId,
        ...
        cmdId = mutils.cmdIdKey(msg.cmdId) || msg.cmdId,
