...
var quickPingWaitTime = config.quickPingWaitTime;
...
function MShepherd(name, settings) {
    ...
    this.authPolicy = {         // All methods in authPolicy can be overrided at will.
        ...
        authorizeForward: function (client, packet, cb) {
            // Default: authorize any packet for any client
            var authorized = true;
            cb(null, authorized);
        }
    };
...
MShepherd.prototype.start = function (callback) {
    ...
    init.setupShepherd(this)
        .timeout(initTimeout,'Broker init timeout.')
        .then(function () {
...
MShepherd.prototype.stop = function (callback) {
    ...
    if (!this._enabled || !this.mClient) {
        deferred.resolve();
        return deferred.promise.nodeify(callback);
    }

    this.permitJoin(0);
    this.announce('stopped').then(function () {
...
MShepherd.prototype.announce = function (msg, callback) {
    var deferred = Q.defer(),
        args = [ 'announce', msg, { qos: 0, retain: false } ];

    Q.npost(this.mClient, 'publish', args).done(deferred.resolve, deferred.reject);
    return deferred.promise.nodeify(callback);
};
...
MShepherd.prototype._requestSender = function (cmdId, clientId, reqObj, waitTime, callback) {
    ...
    if (arguments.length < 2)
        ...
        preError = new Error('No such node.');
    ...
    if (qnode.getStatus() === 'sleep' && !quickping) {
        qnode.quickPingReq().then(function () {
            ...
            if (couldWait)
                ...
                deferred.reject('Client sleeping, clientId: ' + qnode.clientId);
        }).done();
     } else {
        doSending();
     }
