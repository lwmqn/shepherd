...
const debug = require('debug')('lwmqn-shepherd')
const logReq = require('debug')('lwmqn-shepherd:request')
...
function MShepherd (name, settings) {
  ...
  // { clientId: node } box that holds the registered @lwmqn/qnodes
...
MShepherd.prototype.list = function (cIds) {
  ...
  const foundNodes = _.map(cIds, (cid) => {
    let rec
