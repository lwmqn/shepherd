...
MqttNode.prototype.replaceObjectInstance = function (oid, iid, data, callback) {
    ...
    Q.fcall(function () {
        ...
        return mqdb.replace(self.clientId, path, data).then(function (a, b) {
...
MqttNode.prototype.readReq = function (path, callback) {     // path example: oid/iid/rid
    ...
    this.shepherd.readReq.apply(this.shepherd, mutils.turnPathToReqArgs(path, this.clientId)).then(function (msg) {
        var isGoodResponse = mutils.isGoodResponse(msg.status);
        result.status = msg.status;
        result.data = isGoodResponse ? mutils.readDataInfo(path, msg.data).data : msg.data;
...
MqttNode.prototype.writeReq = function (path, data, callback) {
    ...
    var badPath = !(reqDataType === 'object' || reqDataType === 'instance' || reqDataType === 'resource'),
        badData = (reqDataType === 'object' || reqDataType === 'instance') && !_.isObject(data);

    if (badPath || badData) {
        var err = badPath ? new Error('Bad path') : new TypeError('data should be an object.');
        deferred.reject(err);
        return deferred.promise.nodeify(callback);
    }
