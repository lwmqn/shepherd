...
MqttNode.prototype.readReq = function (path, callback) {     // path example: oid/iid/rid
    var self = this,
        ...
        // reqDataType = mutils.requestDataType(path),
    ...
    if (chkErr) {
        ...
        this.shepherd.readReq.apply(this.shepherd, mutils.turnPathToReqArgs(path, this.clientId)).done(function (data) {
            ...
            console.log(data);
            // data = mutils.turnReadDataOfIdKeys(path, data);
            // data = mutils.readDataInfo(path, data).data;
            console.log(data);
            console.log('>>>>>>>>>>> READ DATA END<<<<<<<<<<<<');
            self.status = 'online';
            self._checkAndUpdate(path,  mutils.readDataInfo(path, data).data);
    ...
    if (readyToSend) {
        self.shepherd.writeReq.apply(self.shepherd, mutils.turnPathToReqArgs(path, self.clientId, data)).then(function (rsp) {
            if (rsp.status === 200 || rsp.status === 204) {
                self.status = 'online';
                self._checkAndUpdate(path, data).done();    // immediatley perform local refresh
                self.readReq(path).done();                  // Asynchronously read again
            }
            return rsp;
        }).fail(function (err) {
            deferred.reject(err);
...
MqttNode.prototype._checkAndUpdate = function (path, data, callback) {
    ...
    console.log('XXXXXXXXXXXX READ INFO XXXXXXXXXXXXXXX');
   ...
   //  data = readInfo.data;
    ...
    if (!reqDataType) {
        ...
        switch (reqDataType) {
            case 'object':
                _.forEach(readInfo.data, function (iobj, iid) {
                ...
                break;
            case 'instance':
                console.log('XXXXXXXXXXXX UPDATE6 XXXXXXXXXXXXXXX');

                this.updateObjectInstance(oidkey, iidkey, readInfo.data).done(function (res) {
                    console.log('XXXXXXXXXXXX UPDATE????????? XXXXXXXXXXXXXXX');
                    console.log(res);
                ...
                break;
            case 'resource':
                console.log('XXXXXXXXXXXX UPDATE7 XXXXXXXXXXXXXXX');

                this.updateResource(oidkey, iidkey, ridkey, readInfo.data).done(function (res) {
                    updateFinalizer(null, res);
                }, function (err) {
                    updateFinalizer(err, null);
                });
    ...
    console.log('XXXXXXXXXXXX UPDATE8 XXXXXXXXXXXXXXX');
    return deferred.promise.nodeify(callback);
};
