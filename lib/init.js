...
init.setupShepherd = function (shepherd, callback) {
    var deferred = Q.defer(),
        // Event names for removing broker listeners
        mBrokerEvents = [ 'ready', 'clientConnected', 'clientDisconnecting', 'published', 'subscribed', 'unsubscribed' ],
        broker,
        initProcedure;
...
init._setupAuthPolicy = function (shepherd) {
    ...
    broker.authenticate = function (client, user, pass, cb) {
        ...
        } else if (defaultAccount) {    // if shepherd has a default account, try it
            authorized = (user === defaultAccount.username && pass.toString() === defaultAccount.password);
        }
        ...
        if (authorized) {               // client use a default account and successfully authenticated
            ...
            else                        // if app-level authentication is not implemented, pass all clients
                cb(null, true);
        }
    };
    ...
    broker.on('clientDisconnected', function (client) {
        ...
        if (node) {
            if (node.so) {
                ...
                // node.status = 'offline';
                console.log('init:197:clientDisconnected');
                node.dbSave().done();
            } else {
                shepherd.deregisterNode(client.id, function (err) {
                    console.log(err);
                });
            }
        }
