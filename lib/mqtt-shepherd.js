...
var reqTimeout = config.reqTimeout || 10000,
    tpyTimeout = 8000;
...
function _clientRegisterHandler(shepherd, msg) {
    ...
    var shepherdId = shepherd.clientId,
        acceptedAttrs = [ 'clientId', 'lifetime', 'version', 'objList', 'mac', 'ip' ],
    ...
    if (badAttr) {
        shepherd._responseSender('register', msg.clientId, { status: rspCodeNum('BadRequest') }).done();
        return;
    }
    ...
    if (_.isUndefined(msg.rid)) {   // data is object instance
        ...
        if (!_.isPlainObject(msg.data)) {
            ...
            _.forEach(msg.data, function (val, rid) {
                console.log(rid);
                badResrc = badResrc || !node.getResource(msg.oid, msg.iid, rid);
                console.log(badResrc);
            });
...
function _clientUpdateHandler(shepherd, msg) {
    ...
    if (!isFromRegister && _.has(msg, 'mac')) {
        if (node.mac !== msg.mac) {
            shepherd._responseSender('update', msg.clientId, { status: rspCodeNum('Conflict') }).done();
            return;
        }
    }
    ...
    if (badAttr) {
        shepherd._responseSender('update', msg.clientId, { status: rspCodeNum('BadRequest') }).done();
        return;
    }
    ...
    node.updateAttrs(msg).then(function (diff) {
        ...
        if (_.has(diff, 'objList')) {
            ...
            _clientObjectDetailReq(shepherd, msg.clientId, msg.objList).then(function (objs) {
                ...
                console.log('**** With OBJLIST Update');
                node._registered = true;
...
function _clientResponseHandler(shepherd, msg) {
    ...
    console.log('_clientResponseHandler');
    ...
    // console.log(cmdProm);
    console.log(shepherd._rspsToResolve);
