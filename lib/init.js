...
init._testShepherdPubSub = function (shepherd) {
    var deferred = Q.defer(),
        mc = shepherd.mClient,
        shepherdId = shepherd.clientId,
        testTopics = [ 'register', 'deregister', 'check', 'update', 'notify', 'response', 'ping', 'request', 'announce', 'lwt' ],
    ...
    testMsgListener = function (topic, message, packet) {
        ...
        switch (topic) {
            ...
            case testTopics[2]:    // check/response/shepherdId
...
init._dispatchMessage = function (shepherd, intf, cId, topic, message) {
    var msgStr,
        ...
        objMsgChs = [ 'register', 'deregister', 'check', 'notify', 'update', 'response', 'ping', 'lwt' ];
...
init._attachShepherdMessageHandler = function (shepherd) {
    ...
    mc.on('message', function (topic, message, packet) {
        ...
        // topics: 'register/*', 'deregister/*', 'check/*', notify/*', 'update/*', 'response/*', 'ping'
