    ...
    this.on('_ready', function () {
        ...
        mqdb.find({}).then(function (sos) {
            _.forEach(sos, function (so) {
                ...
                self._sobox[so.clientId] = self.newSmartObject(so.clientId, 'xxx');
...
/*************************************************************************************************/
MShepherd.protoype.newSmartObject = function (clientId, devAttr, options) {
    return new SO(this, clientId, devAttr, options);
};
// ok
...
MShepherd.protoype._registerHandler = function (msg) {
    ...
    if (!so) {
        ...
        so = this.newSmartObject(msg.clientId, msg);
