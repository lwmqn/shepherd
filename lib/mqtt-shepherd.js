...
MShepherd.prototype._requestSender = function (cmdId, clientId, reqObj, callback) {
    ...
    // covert all ids exactly to numbers
    if (_.has(reqObj, 'oid')) {
        var oidNum = parseInt(reqObj.oid);
        oidNum = _.isNumber(oidNum) ? oidNum : reqObj.oid;
        reqObj.oid = oidNum;
    }

    if (_.has(reqObj, 'iid')) {
        var iidNum = parseInt(reqObj.oid);
        iidNum = _.isNumber(iidNum) ? iidNum : reqObj.oid;
        reqObj.iid = iidNum;
    }

    if (_.has(reqObj, 'rid')) {
        var ridNum = parseInt(reqObj.rid);
        ridNum = _.isNumber(ridNum) ? ridNum : reqObj.rid;
        reqObj.rid = ridNum;
    }
...
MShepherd.prototype._setupAuthPolicy = function () {
    ...
    broker.authorizeSubscribe = function (client, topic, cb) {
        ...
        //     cb(null, true);
        // } else if (topic === `+/response/${client.id}`) {
...
MShepherd.prototype._registerHandler = function (msg) {
    ...
    if (!node && msg.clientId) {
        ...
        _.forEach(node.objList, function (iids, oid) {
            ...
            oidNum = _.isNumber(oidNum) ? oidNum : oid;

            var readReqProm = self.readReq(msg.clientId, { oid: oidNum }).then(function (objData) {
                shp('read req response back');
                shp(objData);
