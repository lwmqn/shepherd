...
msghdlr._clientRegisterHandler = function (shepherd, msg) {
    ...
    if (!qnode && msg.clientId) {
        ...
        msghdlr._clientObjectDetailReq(shepherd, msg.clientId, msg.objList).then(function (objs) {
            ...
            emitAtNextTick(shepherd, 'registered', qnode);
            emitAtNextTick(shepherd, 'ind:incoming', qnode);
        }).fail(function (err) {
        ...
        if (qnode.mac !== msg.mac) {
            sendResponse(shepherd, 'register', msg.clientId, transId, 'Conflict');
        } else {
    ...
    qnode.updateAttrs(msg).then(function (diff) {
        ...
        if (_.has(diff, 'objList')) {
            ...
            msghdlr._clientObjectDetailReq(shepherd, msg.clientId, msg.objList).then(function (objs) {
                ...
                emitAtNextTick(shepherd, 'updated', { clientId: qnode.clientId, data: diff });
                emitAtNextTick(shepherd, 'ind:updated', qnode, diff);
            }).fail(function (err) {
                if (!isFromRegister)
...
msghdlr._clientDeregisterHandler = function (shepherd, msg) {
    ...
    sendResponse(shepherd, 'deregister', msg.clientId, msg.transId, 'Deleted', function () {
        emitAtNextTick(shepherd, 'deregistered', msg.clientId);
        emitAtNextTick(shepherd, 'ind:leaving', msg.clientId, macAddr);
    });
};
...
msghdlr._clientNotifyHandler = function (shepherd, msg) {
    ...
    emitAtNextTick(shepherd, 'ind:notified', qnode, msg);
    ...
    qnode._checkAndUpdate(targetPath, msg.data).then(function (diff) {
        ...
        emitAtNextTick(shepherd, 'error', err);
...
/*** Private Helper                                                                            ***/
/*************************************************************************************************/
function sendResponse(shepherd, interface, clientId, transId, status, callback) {
    shepherd._responseSender(interface, clientId, {
        transId: transId,
        status: mutils.rspCodeNum(status)
    }).done(function () {
...
function emitAtNextTick() { // (shepherd, evt, ...)
