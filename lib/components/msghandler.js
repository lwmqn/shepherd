...
const MqttNode = require('./qnode')
const debug = require('debug')('lwmqn-shepherd:msgHdlr')
...
msghdlr._clientUpdateHandler = function (shepherd, msg) {
  ...
  const so = qnode.so
  const oldNodeData = qnode.dump()
  const oldObjList = qnode.objList
...
msghdlr._clientResponseHandler = function (shepherd, msg) {
  ...
  const evt = `${msg.clientId}:${cmdIdString}:${msg.transId}` // 'foo_id:2:101'
