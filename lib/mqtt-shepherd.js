...
function MShepherd(name, settings) {
    ...
    this.permitJoin = function (time, callback) {
        var self = this,
            deferred = Q.defer();
        ...
        if (!this._enabled)
            deferred.resolve(new Error('Shepherd is not enabled.'));
        ...
        deferred.resolve(this._joinable);
...
MShepherd.prototype.maintain = function (cIds, callback) {
    ...
    if (!_.isArray(cIds) || !_.every(cIds, _.isString))
        throw new TypeError('cIds should be an array of strings.');

    _.forEach(nodes, function (qn) {
        if (!qn)
            maintainNodes.push(Q(null));
        else
            maintainNodes.push(qn.maintain());
    });
    ...
    Q.allSettled(maintainNodes).done(function (resArr) {
        ...
        _.forEach(resArr, function (r, i) {
            results.push({
                clientId: cIds[i],
                result: (r.state === 'fulfilled') ? true : false    // not found dev, will be true either
            });
        });
        deferred.resolve(results);
    }, function (err) {
