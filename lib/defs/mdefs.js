...
MDEFS.getOidKey = function (oid) {
    var parsedOidNum = parseInt(oid),
        oidKey;

    if (_.isNumber(parsedOidNum))
        oid = parsedOidNum;

    oidKey = this.OID.get(oid);
...
MDEFS.getRidKey = function (oid, rid) {
    var ridEnum = MDEFS._getRidEnum(oid, rid),
        parsedRidNum = parseInt(rid);

    if (ridEnum && _.isNumber(parsedRidNum))
        rid = ridEnum.get(parsedRidNum).key;

    return _.isUndefined(ridEnum) ? undefined : rid;
};
...
MDEFS.getRidNumber = function (oid, rid) {
    ...
    if (ridEnum && _.isNumber(parsedRidNum))
        rid = ridEnum.get(parsedRidNum).value;
